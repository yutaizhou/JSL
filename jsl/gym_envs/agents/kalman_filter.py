...
class KalmanFilterReg:
    def __init__(
        self, mu0: chex.Array, Sigma0: chex.Array, F: chex.Array, Q: float, R: float
    ):
    ...
    def update(self, X: chex.Array, y: chex.Array):
        def iter_fn(params, carry):
            ...
            C = lambda t: X[t][None, ...]
            kf = KalmanFilter(
                self.F, C, self.Q, self.R, mu.copy(), Sigma.copy(), timesteps=n_obs
            )
            _, (mu_hist, Sigma_hist, _, _) = scan(
                kf.kalman_step, (mu.copy(), Sigma.copy(), 0), y
            )
        ...
        if len(X.shape) == 2:
            ...
            _, (mu_hist, Sigma_hist, _, _) = scan(
                kf.kalman_step, (self.mu.copy(), self.Sigma.copy(), 0), y
            )
            ...
            (mu, Sigma), (mu_hist, Sigma_hist) = scan(
                iter_fn, (self.mu, self.Sigma), (X, y)
            )
        else:
            raise TypeError(
                "The dimension of feature matrix should be greater than or equal to either 2 or 3."
            )
