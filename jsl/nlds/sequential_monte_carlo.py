...
class NonMarkovianSequenceModel:
    ...
    def sample_single(self, key, nsteps):
        ...
        _, hist = jax.lax.scan(
            lambda carry, key: self.sample_step(key, *carry), carry_init, keys
        )
        return hist
    ...
    def sequential_importance_sample(self, key, observations, n_particles=10):
        ...
        init_xparticles = jax.random.normal(
            key_init_particles, shape=(n_particles,)
        ) * jnp.sqrt(self.q)

        carry_init = (init_log_weights, init_mu, init_xparticles)
        xs_tuple = (keys, observations)
        _, dict_hist = jax.lax.scan(
            lambda carry, xs: self._sis_step(xs[0], *carry, xs[1]), carry_init, xs_tuple
        )
        dict_hist["weights"] = jnp.exp(
            dict_hist["log_weights"]
            - jax.nn.logsumexp(dict_hist["log_weights"], axis=1, keepdims=True)
        )
    ...
    def _smc_step(self, key, log_weights_prev, mu_prev, xparticles_prev, yobs):
        ...
        ix_sampled = jax.random.choice(
            key, n_particles, p=weights, shape=(n_particles,)
        )
        xparticles_prev_sampled = xparticles_prev[ix_sampled]
        mu_prev_sampled = mu_prev[ix_sampled]
        # 2. Propagate particles
        xparticles = jax.vmap(self.sample_latent_step)(
            key_particles, xparticles_prev_sampled
        )
        # 3. Concatenate
    ...
    def sequential_monte_carlo(self, key, observations, n_particles=10):
        ...
        init_xparticles = jax.random.normal(
            key_particle_init, shape=(n_particles,)
        ) * jnp.sqrt(self.q)
        ...
        xs_tuple = (keys, observations)
        _, dict_hist = jax.lax.scan(
            lambda carry, xs: self._smc_step(xs[0], *carry, xs[1]), carry_init, xs_tuple
        )
