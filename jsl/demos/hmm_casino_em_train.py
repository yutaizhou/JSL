...
from jsl.hmm.hmm_lib import HMMJax, hmm_em_jax, hmm_sample_jax, init_random_params_jax
...
def main():
    ...
    observations, lens = hmm_utils.hmm_sample_n(
        casino, hmm_sample_jax, n_obs_seq, max_len, rng_sample
    )
    ...
    params_numpy = HMMNumpy(
        np.array(params_jax.trans_mat),
    ...
    start = time.time()
    params_numpy, neg_ll_numpy = hmm_em_numpy(
    ...
    start = time.time()
    params_jax, neg_ll_jax = hmm_em_jax(
        observations, lens, num_epochs=num_epochs, init_params=params_jax
    )
    ...
    dotfile_np = hmm_utils.hmm_plot_graphviz(
        params_numpy.trans_mat, params_numpy.obs_mat, states, observations
    )

    # dotfile_jax = hmm_plot_graphviz(params_jax, "hmm_casino_train_jax", states, observations)
    dotfile_jax = hmm_utils.hmm_plot_graphviz(
        params_jax.trans_mat, params_jax.obs_mat, states, observations
    )
