...
model = tfd.LinearGaussianStateSpaceModel(
    ...
    transition_noise=tfd.MultivariateNormalDiag(scale_diag=step_std * tf.ones([ndims])),
    observation_matrix=tf.linalg.LinearOperatorIdentity(ndims),
    observation_noise=tfd.MultivariateNormalDiag(
        scale_diag=noise_std * tf.ones([ndims])
    ),
    initial_state_prior=tfd.MultivariateNormalDiag(
        loc=jnp.array([5.0]), scale_diag=tf.ones([ndims])
    ),
)
...
mu_hist, Sigma_hist, mu_cond_hist, Sigma_cond_hist = kalman_filter(
    model_lds, jnp.array(y)
)
...
plt.plot(
    smooth_sample[:, :, 0].T, alpha=0.12, color="green"
)  # Kalman smoother backwards sampler.
plt.legend()
plt.show()
