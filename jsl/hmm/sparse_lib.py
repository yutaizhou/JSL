...
def alpha_step(alpha_prev, y, local_evidence_multiple, transition_matrix):
    ...
    carry = {"alpha": alpha_next, "cst": normalisation_cst}
...
def beta_step(beta_next, y, local_evidence_multiple, transition_matrix):
    ...
    carry = {"beta": beta_prev, "cst": norm_cst}
...
def forward_backward(
    ...
    given observation sequence.
    """
    alpha_last, alpha_hist = alpha_forward(
        obs, local_evidence, transition_matrix, alpha_init
    )
    ...
    res = {"filter": filter_hist, "smooth": smooth_hist}
