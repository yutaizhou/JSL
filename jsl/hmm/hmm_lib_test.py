...
def _make_models(init_probs, trans_probs, obs_probs, length):
    ...
    dx_model = HMMJax(trans_probs, obs_probs, init_probs)
...
def test_sample(length, num_states):
    ...
    model, tfp_model = _make_models(
        init_probs, trans_mat, params_fn(num_states), length
    )
...
def test_forward_backward(length, num_states):
    ...
    alphas, betas, marginals, log_prob = hmm_forwards_backwards_jax(model, observations)
