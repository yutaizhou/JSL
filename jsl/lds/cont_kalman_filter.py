...
def sample(
    ...
    * int
    ...
    state_noise = random.multivariate_normal(
        key_state, jnp.zeros(state_size), params.Q, (nsteps,)
    )
    obs_noise = random.multivariate_normal(
        key_obs, jnp.zeros(obs_size), params.R, (nsteps,)
    )
    ...
    sample_obs = (
        jnp.einsum("ij,si->si", params.C, sample_state) + obs_noise[: len(sample_state)]
    )

    return sample_state, sample_obs, jump_size


def filter(params: LDS, x_hist: chex.Array, jump_size: chex.Array, dt: chex.Array):
    """
    ...
    * array(timesteps, state_size, state_size)
    ...
    def step(state, x):
        ...
        (mun, Sigman), _ = lax.scan(
            rk_integration_step, initial_state, jnp.arange(jump_size)
        )
    ...
    _, (mu_hist, Sigma_hist, mu_cond_hist, Sigma_cond_hist) = lax.scan(
        step, initial_state, x_hist[1:]
    )
